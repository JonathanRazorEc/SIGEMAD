...
internal class CreateOrUpdateDireccionCoordinacionCommandHandler : IRequestHandler<CreateOrUpdateDireccionCoordinacionCommand, CreateOrUpdateDireccionResponse>
{
    ...
    //private readonly ICoordinateTransformationService _uTMConversionService;
    ...
    public async Task<CreateOrUpdateDireccionResponse> Handle(CreateOrUpdateDireccionCoordinacionCommand request, CancellationToken cancellationToken)
    {
        ...
        // NUEVO: Procesar geolocalización antes de validaciones geométricas
        foreach (var cecopi in request.CoordinacionesCECOPI ?? new List<CreateOrUpdateCoordinacionCecopiDto>())
        {
            ProcessCoordinacionCECOPIGeolocation(cecopi);
        }

        foreach (var pma in request.CoordinacionesPMA ?? new List<CreateOrUpdateCoordinacionPmaDto>())
        {
            ProcessCoordinacionPMAGeolocation(pma);
        }

        // Validación de geometría existente (ahora con datos procesados)
        foreach (var cecopi in request.CoordinacionesCECOPI ?? new List<CreateOrUpdateCoordinacionCecopiDto>())
        {
    ...
    // nuevos metodos de transformcaion de Geolocalization a UTM
    //CECOPIT
    private void ProcessCoordinacionCECOPIGeolocation(CreateOrUpdateCoordinacionCecopiDto cecopi)
    {
        // Si viene geometría del frontend (GeoJSON), convertir a UTM
        if (cecopi.GeoPosicion != null && (!cecopi.UTM_X.HasValue || !cecopi.UTM_Y.HasValue))
        {
            var (utmX, utmY, huso) = _coordinateTransformationService.ConvertGeometryToUTMDecimal(cecopi.GeoPosicion);
            cecopi.UTM_X = utmX;
            cecopi.UTM_Y = utmY;
            cecopi.Huso = huso;
        }
        // Si vienen coordenadas UTM, generar geometría
        else if (cecopi.UTM_X.HasValue && cecopi.UTM_Y.HasValue && cecopi.Huso.HasValue && cecopi.GeoPosicion == null)
        {
            cecopi.GeoPosicion = _coordinateTransformationService.ConvertUTMToGeometry(
                cecopi.UTM_X, cecopi.UTM_Y, cecopi.Huso);
        }
    }
    ...
    private void ProcessCoordinacionPMAGeolocation(CreateOrUpdateCoordinacionPmaDto pma)
    {
        // Si viene geometría del frontend (GeoJSON), convertir a UTM
        if (pma.GeoPosicion != null && (!pma.UTM_X.HasValue || !pma.UTM_Y.HasValue))
        {
            var (utmX, utmY, huso) = _coordinateTransformationService.ConvertGeometryToUTMDecimal(pma.GeoPosicion);
            pma.UTM_X = utmX;
            pma.UTM_Y = utmY;
            pma.Huso = huso;
        }
        // Si vienen coordenadas UTM, generar geometría
        else if (pma.UTM_X.HasValue && pma.UTM_Y.HasValue && pma.Huso.HasValue && pma.GeoPosicion == null)
        {
            pma.GeoPosicion = _coordinateTransformationService.ConvertUTMToGeometry(
                pma.UTM_X, pma.UTM_Y, pma.Huso);
        }
    }
