...
@Component({
  selector: 'app-ope-punto-control-carretera-filter-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './ope-puntos-control-carreteras-filter-form.component.html',
  styleUrl: './ope-puntos-control-carreteras-filter-form.component.scss',
})
export class OpePuntoControlCarreteraFilterFormComponent implements OnInit {
  @Input() opePuntosControlCarreteras: ApiResponse<OpePuntoControlCarretera[]> | undefined;
  @Input() filtros: any;
  @Input() isLoading: boolean = true;
  @Input() refreshFilterForm: boolean = true;
  @Output() opePuntosControlCarreterasChange = new EventEmitter<ApiResponse<OpePuntoControlCarretera[]>>();
  ...
  public ID_TABLA_LOGS = IDS_TABLAS_LOGS_OPE.PUNTOSCONTROLCARRETERAS;
  // FIN LOGS
  ...
  async ngOnInit() {
    ...
    this.menuItemActiveService.set.emit('/ope-puntos-control-carreteras');
  ...
  */
  ...
  async onSubmit() {
    ...
    this.opePuntosControlCarreterasChange.emit({
      count: 0,
      page: 1,
      pageSize: 10,
      data: [],
    ...
    const opePuntosControlCarreteras = await this.opePuntosControlCarreterasService.get({
    ...
    this.filtrosOpePuntosControlCarreterasService.setFilters(this.formData.value);
    this.opePuntosControlCarreteras = opePuntosControlCarreteras;
    this.opePuntosControlCarreterasChange.emit(this.opePuntosControlCarreteras);
  ...
  goModal() {
    const dialogRef = this.dialog.open(OpePuntoControlCarreteraCreateEdit, {
      ...
      disableClose: true,
      data: {
        title: 'Nuevo - Datos PuntoControlCarretera',
        fire: {},
      },
    });
