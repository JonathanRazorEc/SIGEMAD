...
@Component({
  selector: 'app-ope-datos-asistencias-table',
  standalone: true,
  imports: [
  ...
  templateUrl: './ope-datos-asistencias-table.component.html',
  styleUrl: './ope-datos-asistencias-table.component.scss',
})
export class OpeDatosAsistenciasTableComponent implements OnChanges {
  @Input() opeDatosAsistencias: OpeDatoAsistencia[] = [];
  ...
  public displayedColumns: string[] = [
    'fecha',
    'opePuerto',
    'numeroOpeDatosAsistenciasSanitarias',
    'numeroOpeDatosAsistenciasSociales',
    'numeroOpeDatosAsistenciasTraducciones',
    'numeroOpeDatosAsistenciasTotal',
    'opciones',
  ];

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['opeDatosAsistencias'] && this.opeDatosAsistencias) {
      this.dataSource.data = this.opeDatosAsistencias;
    }
  }
      ...
      this.dataSource.sortingDataAccessor = (item, property) => {
        switch (property) {
          ...
          case 'fecha': {
            ...
            const nombre = (item.opePuerto?.nombre || '').toLowerCase();
          ...
          */
          case 'opePuerto': {
            return item.opePuerto?.nombre || '';
          }
          case 'numeroOpeDatosAsistenciasSanitarias': {
            return this.getNumeroTotalDeListado(item.opeDatosAsistenciasSanitarias);
          }
          case 'numeroOpeDatosAsistenciasSociales': {
            return this.getNumeroTotalDeListado(item.opeDatosAsistenciasSociales);
          }
          case 'numeroOpeDatosAsistenciasTraducciones': {
            return this.getNumeroTotalDeListado(item.opeDatosAsistenciasTraducciones);
          }
          case 'numeroOpeDatosAsistenciasTotal': {
            return this.getNumeroTotalAsistencias(
              item.opeDatosAsistenciasSanitarias,
              item.opeDatosAsistenciasSociales,
              item.opeDatosAsistenciasTraducciones
            );
          }
          default: {
            const result = item[property as keyof OpeDatoAsistencia];
        ...
        return data.sort((a, b) => {
          if (sort.active === 'fecha') {
            ...
            if (result === 0) {
              ...
              const nombreA = (a.opePuerto?.nombre || '').toLowerCase();
              const nombreB = (b.opePuerto?.nombre || '').toLowerCase();
  ...
  /*
  goToEdit(frontera: OpeDatoAsistencia) {
    //this.router.navigate([`fire/fire-national-edit/1`]);
  }
  */

  goToEditDatoAsistencia(opeDatoAsistencia: OpeDatoAsistencia) {}
  ...
  goModalEdit(opeDatoAsistencia: OpeDatoAsistencia) {
    const dialogRef = this.dialog.open(OpeDatoAsistenciaCreateEdit, {
      width: '65vw',
      maxWidth: 'none',
      disableClose: true,
      data: {
        title: 'Modificar - Dato de asistencia.',
        opeDatoAsistencia: JSON.parse(JSON.stringify(opeDatoAsistencia)), // clonamos el objeto para evitar problemas de referencia
      },
    });
  ...
  getNumeroTotalAsistencias(sanitarias: any[] | undefined, sociales: any[] | undefined, traducciones: any[] | undefined): number {
    return this.getNumeroTotalDeListado(sanitarias) + this.getNumeroTotalDeListado(sociales) + this.getNumeroTotalDeListado(traducciones);
  }
}
